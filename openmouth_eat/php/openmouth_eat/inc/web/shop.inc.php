...
if ($op=='post'){
    ...
    $id = $_GPC['id'];
    if ($id){
        $list = pdo_fetch('SELECT * FROM '.tablename('openmouth_eat_shop').' WHERE id=:id',array(':id'=>$id));
    }
    if($_W['ispost']){
		if($_GPC['seller_name'] == ""){
			message("名称不得为空");
        ...
        if ($id){
			...
			$res = pdo_insert('openmouth_eat_shop',$data);
            message('添加成功',$this->createWebUrl('shop',array('op'=>'display')));
        }
    }
} elseif ($op=='displayall'){
    $page = max(1,intval($_GPC['page']));
    $pagesize = 10;
    $lists = pdo_fetchall('SELECT * FROM '.tablename('openmouth_eat_shop').' WHERE uniacid =:uniacid ORDER BY `id` ASC LIMIT '.($page - 1) * $pagesize . "," . $pagesize,array(':uniacid'=>$_W['uniacid']));	
